...
object SignTool {
    ...
    fun printSignatureMD5(
        ...
        LogUtil.d("${AppUtil.AppInfo.getApplicationName(mContext)}    启动成功 " +
                "\nAPP包名       => ${AppUtil.AppInfo.getPackageName()} " +
                "\nAPP版本名称    => ${AppUtil.AppInfo.getAppVersionName(mContext)} " +
                "\nAPP版本号      => ${AppUtil.AppInfo.getAppVersionCode(mContext)} " +
                "\n32位 MD5 签名  => ${getMD5MessageDigest(mContext)}"+
                "\n当前手机CPU     => ${Build.CPU_ABI}" +
                "\n异常目录       =>${ConfigBuilder.Path.CRASH_LOG_PATH}")
    }
